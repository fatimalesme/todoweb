...
$tiene_sesion = isset($_SESSION['usuario_id']) || (isset($_SESSION['rol']) && $_SESSION['rol'] === 'guest');
if (!$tiene_sesion) {
        ...
        if ($rol === 'guest') {
            if (isset($_SESSION['tareas_invitado'][$tarea_id])) {
                $_SESSION['tareas_invitado'][$tarea_id]['pomodoro_segundos'] =
                    ($_SESSION['tareas_invitado'][$tarea_id]['pomodoro_segundos'] ?? 0) + $segundos;
                echo json_encode(['success' => true]);
                exit();
            }
        } else {
            $stmt = $conexion->prepare(
                'UPDATE tareas SET pomodoro_segundos = pomodoro_segundos + ? WHERE id = ? AND id_usuario = ?'
            );
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['tarea_id'])) {
    ...
    if ($rol === 'guest') {
        $seg = $_SESSION['tareas_invitado'][$tarea_id]['pomodoro_segundos'] ?? 0;
        echo json_encode(['segundos' => $seg]);
        exit();
    } else {
        $stmt = $conexion->prepare(
            'SELECT pomodoro_segundos FROM tareas WHERE id = ? AND id_usuario = ?'
        );
